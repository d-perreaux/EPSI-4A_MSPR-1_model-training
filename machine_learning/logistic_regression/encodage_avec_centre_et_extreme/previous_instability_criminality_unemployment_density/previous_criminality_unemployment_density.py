...
query = """
    SELECT 
    encodage_centre_extremes_gagnant as prediction, encodage_centre_extremes_gagnant_precedent as precedent, criminality_per_cir.code_circonscription as code_circonscirption,
    taux_pour_mille, taux_chomage, density, instabilite_centre_extremes, poids_nuance_centre_extremes, desir_changement_centre_extremes 
...
X = df[['precedent', 'taux_pour_mille', 'taux_chomage', 'density', 'instabilite_centre_extremes', 'poids_nuance_centre_extremes', 'desir_changement_centre_extremes']]
y = df.prediction
labels = [1, 2, 3, 4, 5]
...
plt.figure()
sns.histplot(y_hat_proba[:, 4], kde=True, label="Classe 5")  # Classe 1
plt.legend()
plt.savefig(f'histplot.predict_proba_5.png')
...
print(np.min(y_hat_proba[:, 4]), np.max(y_hat_proba[:, 4]))

        ...
        if i == len(years) - 1:
            ...
            for result in results_job:
                if (result['annee'] == year) and (result['code_de_la_circonscription'] == code_circumscription):
                    actual_winner = result['encodage_centre_extremes_gagnant']
                    winner_n_1 = next((r['encodage_centre_extremes_gagnant'] for r in results_job if r['annee'] == (years[i+1]) and r['code_de_la_circonscription'] == code_circumscription))
    ...
    for i, year in enumerate(years_to_process):
        for result in results_job:
            if (result['annee'] == year) and (result['code_de_la_circonscription'] == code_circumscription):
                ...
                actual_winner = result['encodage_centre_extremes_gagnant']
                winner_n_1 = next((r['encodage_centre_extremes_gagnant'] for r in results_job if r['annee'] == (years[i+1]) and r['code_de_la_circonscription'] == code_circumscription))
                winner_n_2 = next((r['encodage_centre_extremes_gagnant'] for r in results_job if
                                   r['annee'] == (years[i + 2]) and r[
                                       'code_de_la_circonscription'] == code_circumscription))
                winner_n_3 = next((r['encodage_centre_extremes_gagnant'] for r in results_job if
                                   r['annee'] == (years[i + 3]) and r[
                                       'code_de_la_circonscription'] == code_circumscription))
                winner_n_4 = next((r['encodage_centre_extremes_gagnant'] for r in results_job if
                ...
                SET instabilite_centre_extremes = %s, poids_nuance_centre_extremes = %s, desir_changement_centre_extremes = %s
